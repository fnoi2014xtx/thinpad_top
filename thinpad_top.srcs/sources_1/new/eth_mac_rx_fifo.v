...
always @(negedge clk) begin
    ...
    case (state_in)
        STATE_IDLE: begin         //初始状态，并未读取数据
            if(s_axis_tvalid_reg == 1'b1)begin
                if(buffer_tail+1==buffer_head||(buffer_tail==BUFFER_N&&buffer_head==0)||s_axis_tuser_reg==1'b1) //buffer满了
                    state_in = s_axis_tlast_reg?STATE_IDLE:STATE_DROP;//如果byte1即为最后byte，直接舍弃，否则进入drop模式
                else begin
                    frame_st[frame_tail] = buffer_tail;         //创建一帧，当读取完该帧全部内容后，frame_tail+1
                    ...
                    if(s_axis_tlast_reg == 1'b1)begin
                        ...
                        state_in =  STATE_TRANSFER; //检查第一帧有没有错误
                end
            end
        end 
        STATE_TRANSFER: begin
            if(s_axis_tvalid_reg == 1'b1)begin
                if(buffer_tail+1==buffer_head||(buffer_tail == BUFFER_N && buffer_head == 0) || s_axis_tuser_reg == 1'b1)begin
                    state_in = s_axis_tlast_reg?STATE_IDLE:STATE_DROP;
